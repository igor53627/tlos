...
/**
 * @title TLOSKeccak (DEPRECATED - Classical only, NOT post-quantum)
 *
 * @notice This contract uses keccak256 for SEH and is NOT post-quantum secure.
 *         For any post-quantum use-case, use TLOSLWE.sol instead.
 *
 * @dev This contract is kept only for historical/benchmarking purposes
 *      and may be removed in a future major release.
 *
 *      Keccak SEH provides ~128-bit classical security but only ~64-bit
 *      post-quantum security due to Grover's algorithm.
 */

import "../interfaces/IHoneypot.sol";
import {SSTORE2} from "solmate/utils/SSTORE2.sol";

/// @custom:deprecated Use TLOSLWE for post-quantum deployments
contract TLOSKeccak is IHoneypot {
    ...
    bytes32 public immutable expectedSehHash;  // NEW: SEH consistency hash
    ...
    constructor(
        ...
        expectedSehHash = _expectedSehHash;
    ...
    /// @notice Check with SEH hash for debugging
    function checkWithSeh(bytes32 input) external view returns (bool valid, bytes32 sehHash) {
    ...
    function scheme() external pure override returns (string memory) { return "tlos-keccak"; }
    function encryptedGates() external pure override returns (uint256) { return 640; }
    function estimatedGas() external pure override returns (uint256) { return 3_500_000; }
    ...
    function _evaluate(bytes32 input) internal view returns (bool valid, bytes32 sehHash) {
        ...
        // Initialize SEH accumulator with input and initial wires
        ...
        // Process gates in batches for SEH updates (batch size = 64 for gas efficiency)
        ...
        for (uint256 batchStart = 0; batchStart < gateCount; batchStart += batchSize) {
            ...
            assembly {
                let dataPtr := add(add(cd, 32), mul(batchStart, 523))
                let endPtr := add(add(cd, 32), mul(batchEnd, 523))
            ...
            // Update SEH after each batch (per-batch binding for gas efficiency)
            // Binds: previous sehAcc + batch index + current wire state
            sehAcc = keccak256(abi.encodePacked(sehAcc, batchEnd, wires));
        }
        
        // Final SEH hash includes the accumulated state
        sehHash = sehAcc;
        
        bytes32 outputHash = keccak256(abi.encodePacked(wires));
        valid = (outputHash == expectedOutputHash) && (sehHash == expectedSehHash);
    }
