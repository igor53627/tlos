...
\title{\tlos{}: Topology-Lattice Obfuscation with SEH\\for Smart Contracts}
\author{}
\date{}
...
\begin{abstract}
We present \tlos{} (Topology-Lattice Obfuscation with SEH), an extension of \tlo{} that adds inter-gate wire consistency via Subspace-Evasive Hashing (SEH) from Ma-Dai-Shi 2025~\cite{MDS25}. \tlos{} uses a three-layer security model: a topology layer that defeats structural and statistical attacks, a lattice layer using on-chain \lwe{} inner products that defeats semantic attacks, and an SEH layer that binds wire values across gates to prevent mix-and-match attacks.

\tlos{} achieves 6/6 resistance against our attack evaluation matrix at ${\sim}2.59$M gas (Keccak SEH) or ${\sim}3.29$M gas (LWE SEH) with $n{=}64$ \lwe{} dimension. The SEH layer adds only 0.5\% overhead for Keccak-based binding or 27.7\% for full LWE-based binding, both well within EVM block limits. Control functions are hidden via \lwe{} ciphertexts; SEH ensures that gate evaluations cannot be mixed-and-matched across different execution paths. Target applications include predicates with \emph{eventually-expiring} secrets (honeypots, sealed-bid auctions, lotteries, MEV protection). Deployment requires only a standard smart contract with timestamp-based expiry.
\end{abstract}
...
\begin{enumerate}
  ...
  \item \textbf{Layer 3 (SEH):} Subspace-Evasive Hashing binds wire values across gate evaluations, preventing mix-and-match attacks. \emph{Security: collision resistance (Keccak variant) or LWE subspace evasion (LWE variant).}
\end{enumerate}

\textbf{Key mechanism:} Wrong-key decryption yields garbage; SEH ensures consistent execution traces.
}}
\end{center}
...
\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{Variant} & \textbf{Operation} & \textbf{Gas ($n{=}64$)} & \textbf{\% of 30M} \\
\midrule
\tlo{} (base) & \texttt{check()} & 2.58M & 8.6\% \\
\tlos{} (Keccak SEH) & \texttt{check()} & 2.59M & 8.6\% \\
\tlos{} (LWE SEH) & \texttt{check()} & 3.29M & 11.0\% \\
\midrule
All & \texttt{commit()} & 47K & 0.16\% \\
All & \texttt{reveal()} & varies & varies \\
\bottomrule
\end{tabular}
\caption{Gas costs on 64-wire/640-gate circuits with $n{=}64$ \lwe{} dimension. Tenderly-measured.}
\end{table}
...
%------------------------------------------------------------------------------
\section{SEH: Inter-Gate Wire Binding}
\label{sec:seh}
%------------------------------------------------------------------------------

\tlos{} extends \tlo{} with Subspace-Evasive Hashing (SEH) from Ma-Dai-Shi 2025~\cite{MDS25}. SEH binds wire values across gate evaluations, preventing mix-and-match attacks.

\subsection{The Mix-and-Match Attack}

Without SEH, gates are evaluated independently. An attacker could potentially:
\begin{enumerate}
...
\subsection{SEH Construction}

SEH creates a cryptographic chain linking all gate evaluations:
\begin{align*}
h_0 &= H(\mathsf{seed} \| \mathsf{input}) \\
h_i &= H(h_{i-1} \| \mathsf{gate}_i\text{-}\mathsf{output}) \quad \text{for } i = 1, \ldots, N
\end{align*}

Each gate must prove its inputs came from the correct predecessor state. Forged inputs produce different $h_i$ values, propagating to an incorrect final hash.

\subsection{Two SEH Variants}

\begin{center}
\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{Variant} & \textbf{Gas} & \textbf{Overhead} & \textbf{Security Basis} \\
\midrule
Keccak SEH & 2.59M & +0.5\% & Collision resistance \\
LWE SEH & 3.29M & +27.7\% & LWE subspace evasion \\
\bottomrule
\end{tabular}
\end{center}

\textbf{Keccak SEH} uses $\mathsf{keccak256}$ for the hash chain. Security relies on collision resistance (${\sim}128$-bit classical, weaker post-quantum due to Grover).

\textbf{LWE SEH} uses an LWE matrix-vector product: $H(x) = Ax \bmod q$ where $A$ is derived from the circuit seed. Security relies on LWE subspace evasion.

\subsection{Security Properties}

\textbf{What SEH provides:}
\begin{itemize}
  \item Mix-and-match prevention: Gates cannot be evaluated with inconsistent inputs
  \item Execution trace binding: Full evaluation history is committed
  \item Formal s-equivalence (LWE SEH): Provable consistency between equivalent circuits
\end{itemize}

\textbf{What SEH does NOT provide:}
\begin{itemize}
  \item Semantic security: Black-box evaluation is still possible
  \item Full iO: This is not indistinguishability obfuscation
  \item Key extraction resistance: Still relies on the \lwe{} layer
\end{itemize}
...
\begin{itemize}
  \item \textbf{LWE} for control function hiding (${\sim}49$-bit post-quantum for $n{=}64$)
  \item \textbf{SEH} for wire consistency (Keccak: ${\sim}128$-bit classical; LWE: ${\sim}49$-bit PQ)
  \item \textbf{Hash preimage} for unlocking (${\sim}256$-bit)
\end{itemize}

The weakest link determines overall security. For Keccak SEH, the post-quantum claims are limited by Keccak's Grover vulnerability. For LWE SEH, all three layers have consistent post-quantum security.
...
\begin{itemize}
  ...
  \item \textbf{\lwe{} layer (computational):} On-chain inner product computation hides control functions, defeating semantic attacks. ${\sim}108$-bit classical security with $n{=}64$ using uniform secrets.
  \item \textbf{SEH layer (wire binding):} Subspace-Evasive Hashing from Ma-Dai-Shi~\cite{MDS25} binds wire values across gates, preventing mix-and-match attacks.
\end{itemize}

\tlos{} achieves 6/6 resistance against our attack evaluation matrix at ${\sim}2.59$M gas (Keccak SEH) or ${\sim}3.29$M gas (LWE SEH), with $n{=}64$ and 64 wires/640 gates. SEH adds only 0.5\%--27.7\% overhead depending on variant. Security is based on \lwe{} hardness, collision resistance (Keccak) or LWE subspace evasion (LWE SEH), and empirical topology properties. The system targets \emph{eventually-expiring secrets} where moderate security suffices for the secret's lifetime.
...
\begin{itemize}
  ...
  \item SEH wire binding: inter-gate consistency prevents mix-and-match attacks
...
\textbf{Limitations:} Topology security is heuristic, not proven. Keccak SEH has weaker post-quantum properties than LWE SEH. Gas costs may be prohibitive for some applications.

\textbf{Future Work:}
\begin{itemize}
  \item Per-gate SEH updates for stronger binding
  \item Formal analysis of topology layer security
  \item Hybrid Keccak/LWE SEH for gas-security trade-offs
\end{itemize}
