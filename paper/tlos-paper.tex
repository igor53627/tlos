...
\begin{enumerate}
  \item \textbf{\tlos{} Framework:} A three-layer obfuscation framework for EVM combining topology-based structural mixing, on-chain \lwe{} inner products, and a full-rank linear SEH layer. Our implementation uses $n{=}128$ LWE dimension and a $64{\times}64$ SEH matrix, achieving ${\sim}8.5$M gas (28\% of block) and 662\,KB storage for 640 gates with 6/6 resistance in our attack evaluation matrix.
...
\tlos{} achieves 6/6 resistance against our attack evaluation matrix at ${\sim}8.5$M gas with $n{=}128$ and 64 wires/640 gates. Security is based on \lwe{} hardness; SEH provides algebraic wire binding. The system targets \emph{eventually-expiring secrets} where post-quantum security is desired.
