...
\begin{itemize}
  \item \textbf{Human-memorable recovery codes:} Smart wallets can use 6-word recovery phrases protected by TLOS. With keccak, a $10^6$-word dictionary is cracked in $<$1ms; with TLOS Layer 4 puzzle, the attacker must solve a $2^{76}$ ternary LWE puzzle \emph{per guess}, making even small dictionaries computationally infeasible.
...
\begin{itemize}
  ...
  \item \textbf{Sealed-bid auctions:} Bids from small domains (0--10,000 price levels) cannot be dictionary-attacked. Each guess requires solving the Layer 4 puzzle ($2^{76}$ work).
...
\begin{itemize}
  ...
  \item \textbf{One-time delegation codes:} Short numeric codes (6 digits) grant temporary access. Layer 4 puzzle forces $2^{76}$ work per guess, making brute-force infeasible regardless of code entropy.
...
\begin{enumerate}
  ...
  \item Generate puzzle: plant ternary secret $s^* \in \{-1,0,1\}^{48}$, derive $(A,b)$ where $b = As^* + e$
  \item Deploy contract with circuit data (SSTORE2), puzzle parameters, expected output hash, and expiry timestamp
\end{enumerate}
...
\begin{enumerate}
  ...
  \item Solve puzzle: find $s$ such that $\|As - b\|_\infty < \tau$ (off-chain computation)
...
\begin{enumerate}
  ...
  \item Resort to brute force (Layer 4 puzzle forces $2^{76}$ work per guess; even low-entropy inputs protected)
\end{enumerate}
...
\begin{enumerate}
  \item \textbf{Commit:} User submits $H = \mathsf{hash}(\mathsf{sender} \| \mathsf{guess})$
  \item \textbf{Wait:} $k \geq 1$ blocks
  \item \textbf{Solve puzzle:} Find $s$ such that $\|As - b\|_\infty < \tau$ (off-chain)
  \item \textbf{Reveal:} User submits (guess, puzzleSolution); contract verifies $H$ matches AND puzzle valid
\end{enumerate}

Sender binding prevents reward theft: an attacker copying the reveal transaction computes $\mathsf{hash}(\mathsf{attacker} \| \mathsf{guess}) \neq H$. The puzzle adds $2^{76}$ minimum work per guess attempt.
