...
\begin{enumerate}
  ...
  \item \textbf{Layer 2 (LBLO):} On-chain inner product computation defeats semantic attacks (RainbowTable). Control functions hidden via noiseless LWE-like ciphertexts. \emph{Security: heuristic ${\sim}2^{120\text{--}140}$ PQ cost for known attacks on our LBLO construction with $n{=}768$; not standard LWE (see \S\ref{sec:lblo}).}
...
\begin{enumerate}
  \item \textbf{\tlos{} Framework:} A three-layer obfuscation framework for EVM combining topology-based structural mixing, on-chain LBLO (noiseless LWE-like) inner products, and a full-rank wire binding layer. Our implementation uses $n{=}768$ LWE dimension and a $64{\times}64$ binding matrix, achieving 10.5M--38.1M gas (17--63\% of block) and 11 bytes/gate storage (seed-derived $\vec{a}$ vectors) with 9/9 resistance in our attack evaluation matrix.
  ...
  \item \textbf{Post-Quantum Resistance (Conjectured):} Security of control-function hiding relies on the conjectured hardness of LBLO. Our ${\sim}2^{120\text{--}140}$ PQ estimate for $n{=}768$ is based on heuristic modeling with the LWE estimator (see Table~\ref{tab:lwe-security}), not on standard LWE reductions.
...
\begin{itemize}
  ...
  \item Practical gas costs (10.5M--38.1M for $n{=}768$, 17--63\% of 60M block limit)
...
\begin{itemize}
  ...
  \item \textbf{Gas cost is acceptable:} At 0.03 gwei, 10.5M--38.1M gas $\approx$ \$1--\$4, negligible for most applications
\end{itemize}
...
For long-lived secrets, an adversary can analyze the circuit for arbitrarily long time. While our LBLO construction provides estimated computational security (${\sim}2^{120\text{--}140}$ PQ for $n{=}768$, heuristic), we do not claim this is sufficient for secrets requiring decades of protection.
...
\lwe{} is believed quantum-resistant and forms the basis for post-quantum cryptography standards including ML-KEM~\cite{FIPS203}. Our deployed parameter ($n{=}768$, $q{=}65521$) exceeds NIST ML-KEM-768 dimension. See Table~\ref{tab:lwe-security} for security estimates based on the LWE Estimator~\cite{APS15}.
...
\begin{itemize}
  ...
  \item Dimension: $n = 768$ (deployed); $n \in \{16, 32, 128\}$ for historical experiments
  \item Sample count: $m = 256$--$1024$ (4 ciphertexts $\times$ 64--256 gates)
...
\begin{itemize}
  \item $n{=}128$ (old): 1035 bytes/gate, 662 KB total (640 gates)
  \item $n{=}768$ (seed-derived): \textbf{11 bytes/gate}, 2.8 KB total (256 gates)
\end{itemize}
...
$n{=}128$ & ${\sim}203$-bit & ${\sim}98$-bit & 8.5M & Historical parameter (see $n{=}768$) \\
\textbf{$n{=}768$} & \textbf{${\sim}280$-bit} & \textbf{${\sim}120$--$140$-bit} & \textbf{10.5M--38.1M} & \textbf{Deployed parameter (production use)} \\
\bottomrule
\end{tabular}
\caption{LBLO security estimates (modeled as uniform-secret LWE; $q{=}65521$, no error term). Estimates based on dual attacks in the LWE Estimator~\cite{APS15}; \textbf{heuristic} since our construction lacks noise and standard LWE reductions do not apply. Gas costs for $n{=}768$: 64 gates = 10.5M, 128 gates = 19.3M, 256 gates = 38.1M.}
\end{table}
...
\midrule
$n{=}128$ (historical) & ${\sim}98$-bit & 640 & 8.5M & 14\% \\
\textbf{$n{=}768$} & \textbf{${\sim}140$-bit} & 64 & 10.5M & 17\% \\
\textbf{$n{=}768$} & \textbf{${\sim}130$-bit} & 128 & 19.3M & 32\% \\
\textbf{$n{=}768$} & \textbf{${\sim}120$-bit} & 256 & 38.1M & 63\% \\
\bottomrule
\end{tabular}
\caption{\tlos{} gas costs by configuration (Tenderly-confirmed, 60M block limit). Security estimates for uniform-secret LBLO (dual attack). Fewer gates = fewer LWE samples = higher security.}
\end{table}
...
\begin{tabular}{@{}lcccc@{}}
\toprule
\textbf{Config ($n{=}768$)} & \textbf{Gates} & \textbf{Storage} & \textbf{Old Format} & \textbf{Savings} \\
\midrule
...
\begin{enumerate}
  \item \textbf{Deploy circuit data (one-time):} Store gate topology and $b$ values via SSTORE2. For 256 gates at $n{=}768$: ${\sim}2.8$ KB, fits in single SSTORE2 contract (${\sim}615$K gas).
...
\texttt{reveal(input)} / \texttt{mint()} & tx & 38.1M & Executes \texttt{\_evaluate()} \\
SSTORE2 deploy & tx & ${\sim}615$K & One-time, shared \\
\bottomrule
\end{tabular}
\caption{Gas costs by operation type ($n{=}768$, 256 gates).}
\end{table}

\textbf{Implications:}
\begin{itemize}
  \item \textbf{Attackers} can brute-force \texttt{check()} locally without paying gas, but each guess still incurs ${\sim}100$ms computational cost (the LBLO slowdown).
  \item \textbf{Legitimate users} pay 38.1M gas only once when executing the state-changing operation (\texttt{reveal()}, \texttt{mint()}, etc.).
\end{itemize}
...
\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{Config ($n{=}768$)} & \textbf{Gates} & \textbf{Evaluation Gas} & \textbf{Block \%} \\
\midrule
Conservative & 64 & 10.5M & 17\% \\
Balanced & 128 & 19.3M & 32\% \\
Full security & 256 & 38.1M & 63\% \\
\bottomrule
\end{tabular}
\caption{Gas costs with seed-derived $\vec{a}$ vectors (Tenderly-confirmed, 60M block limit).}
\end{table}

\textbf{Security vs.\ gas tradeoff:} Fewer gates = fewer LWE samples = higher security but less circuit mixing. For $n{=}768$:
\begin{itemize}
  \item 64 gates (256 samples): ${\sim}140$+ bit security, 10.5M gas
  \item 128 gates (512 samples): ${\sim}130$+ bit security, 19.3M gas  
  \item 256 gates (1024 samples): ${\sim}120$+ bit security, 38.1M gas
\end{itemize}
...
\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{Variant} & \textbf{Operation} & \textbf{Gas ($n{=}768$)} & \textbf{\% of 60M} \\
\midrule
\tlos{} (64 gates) & \texttt{check()} & 10.5M & 17\% \\
\tlos{} (128 gates) & \texttt{check()} & 19.3M & 32\% \\
\tlos{} (256 gates) & \texttt{check()} & 38.1M & 63\% \\
\midrule
All & \texttt{commit()} & 47K & 0.08\% \\
All & \texttt{reveal()} & varies & varies \\
\bottomrule
\end{tabular}
\caption{Gas costs with $n{=}768$ \lwe{} dimension and seed-derived $\vec{a}$ vectors. Tenderly-measured.}
\end{table}
...
\begin{itemize}
  \item \textbf{LBLO} for control function hiding (${\sim}120$--$140$-bit post-quantum for $n{=}768$)
...
This costs ${\sim}50$K gas and provides 256-bit preimage resistance. Why use \tlos{}'s 10.5M--38.1M gas for ${\sim}120$--$140$-bit LBLO security?

\subsection{When Hash-Compare Suffices}

For simple ``guess the secret'' predicates, \textbf{hash-compare is equivalent or better}:
\begin{itemize}
  \item Lower gas (${\sim}50$K vs 10.5M--38.1M)
  \item Higher security (256-bit vs 120--140-bit PQ)
...
\begin{enumerate}
  \item \textbf{Gas costs:} \tlos{} with $n{=}768$ requires 10.5M--38.1M gas (17--63\% of 60M block limit). This is practical for high-value, infrequent operations.
...
\begin{itemize}
  ...
  \item \textbf{LBLO layer (heuristic):} On-chain inner product computation hides control functions, defeating semantic attacks. \emph{Estimated} ${\sim}2^{120\text{--}140}$ post-quantum cost for known attacks with $n{=}768$; this is a heuristic estimate based on the LBLO problem (\S\ref{sec:lblo}), not a reduction to standard LWE.
...
\tlos{} achieves 9/9 resistance against our attack evaluation matrix at 10.5M--38.1M gas with $n{=}768$ and 64--256 gates. Security is based on conjectured LBLO hardness; the wire binding layer provides algebraic consistency. The system targets \emph{eventually-expiring secrets} where post-quantum security is desired.
...
\textbf{Limitations:} Topology security is heuristic, not proven. LBLO security is conjectured based on empirical attack analysis, not proven via reduction to standard assumptions. Gas costs (10.5M--38.1M) may be prohibitive for frequent operations. We do not recommend current parameters for high-value, long-lived secrets until further independent cryptanalysis validates the LBLO hardness conjecture.
