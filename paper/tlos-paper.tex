...
\textbf{Brute-force resistance:} The search space is $3^{48} \approx 2^{76}$. At 436M guesses/sec (NVIDIA GH200 benchmark), exhaustive search requires:
\begin{itemize}
\item 1 GPU: ${\sim}5.7$ million years
\item 100 GPUs: ${\sim}57,000$ years  
\item 10,000 GPUs: ${\sim}570$ years
\end{itemize}

\textbf{Lattice attacks:} Standard BKZ/LLL attacks on LWE with these parameters have complexity exceeding brute-force due to the small dimension but ternary secret structure. Meet-in-the-middle attacks reduce effective security to approximately $2^{40}$--$2^{50}$, still requiring years on GPU clusters.
