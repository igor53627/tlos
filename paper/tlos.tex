...
\item \textbf{\lwe{}:} On-chain inner products hide control functions. \emph{Security: estimated ${\sim}2^{98}$ PQ with $n{=}128$.}
\item \textbf{SEH:} Full-rank linear hash binds wire values across gates, preventing mix-and-match. \emph{Security: algebraic binding (bijective map).}
\end{enumerate}
}}
\end{center}
...
\item \textbf{Full-Rank Linear Hash:} Inter-gate wire binding via on-chain $64{\times}64$ matrix-vector products. Inspired by~\cite{MDS25} but simplified: a public bijective map providing algebraic binding, not cryptographic hiding.
\item \textbf{Per-Batch Updates:} Gas-efficient binding (128-gate batches, 5 updates for 640 gates).
\item \textbf{SEH PRG Optimization:} Derive 16 matrix coefficients per keccak call (320 calls vs 4096).
...
\bottomrule
\end{tabular}
\caption{Offline brute-force cost comparison. \textsuperscript{$\dagger$}For high-entropy secrets, \tlos{} security is bounded by LWE hardness (${\sim}2^{98}$ with $n{=}128$). Assumptions: keccak at $10^9$ hashes/s, \tlos{} at 100\,ms/guess.}
...
Under these constraints, \tlos{} provides the \emph{only} way to make low-entropy secret verification expensive on EVM---no memory-hard KDF (Argon2/scrypt) exists as a precompile.

\subsection{Why Not Keccak?}

\textbf{Low-entropy secrets.} Keccak can be brute-forced at ${\sim}1$ns/guess. \tlos{} transforms each guess into expensive LWE computation (${\sim}100$ms/guess), a $10^8\times$ slowdown.
...
\subsection{Full-Rank Linear SEH}
...
function _sehHash(uint256 input, uint256 gateIdx) 
...
This provides 5 hash updates for 640 gates. Combined with the PRG optimization (16 coefficients per keccak instead of 1), total SEH overhead is reduced by ${\sim}25\times$ compared to naive implementation.
...
\textbf{Security estimate (heuristic):} Our implementation does \emph{not} add an independent error term, so this is not standard LWE. The ${\sim}2^{98}$ PQ estimate is based on heuristic modeling as LWE$(n{=}128, q{=}65521, m{=}2560)$ with small error using the lattice estimator~\cite{APS15}. This should be interpreted as a rough complexity yardstick, not a rigorous reduction.
...
\noindent\textbf{Caveat:} Absence of known attacks $\neq$ proven security. The ${\sim}2^{98}$ estimate is a heuristic lower bound.
...
\midrule
CF hiding & \lwe{}-like ($n{=}128$, $q{=}65521$, $m{=}2560$) & ${\sim}2^{98}$ PQ (heuristic) \\
SEH binding & Full-rank 64$\times$64 linear map & Algebraic binding \\
Topology & Heuristic & Empirical (\S\ref{sec:topology-exp}) \\
\bottomrule
\end{tabular}
\caption{Security profile. LWE estimate is heuristic (see \S\ref{sec:lwe-params}). SEH provides binding via trivial kernel.}
\label{tab:security}
\end{table}

We estimate ${\sim}2^{98}$ post-quantum cost against the underlying \lwe{} instances with $n{=}128$ using standard lattice estimators. This security level is suitable for:
\begin{itemize}
...
\begin{itemize}
\item \textbf{TLOS-LWE ($n{=}128$, full-rank SEH)}: ${\sim}8.5$M gas (28\% of block)
\end{itemize}

\textbf{Optimizations applied:}
\begin{itemize}
\item SEH PRG: 16 coefficients per keccak (320 calls vs 4096 per update)
\item Single \texttt{mod} at end of inner product (vs per-term)
\item Batch size 128 (5 SEH updates for 640 gates)
...
Topology + \lwe{} & 3/3 & 1/1 & 0/1 \\
Topology + \lwe{} + SEH & 3/3 & 1/1 & 1/1 \\
\bottomrule
\end{tabular}
\end{center}

\textbf{Key findings:} (1)~Topology alone defeats structural attacks but not semantic (RainbowTable) attacks. (2)~\lwe{} is necessary to hide control functions from semantic analysis. (3)~SEH is necessary to prevent mix-and-match---without it, an attacker can evaluate gates with inconsistent wire values.

%------------------------------------------------------------------------------
\section{Conclusion}
%------------------------------------------------------------------------------

\tlos{} is a practical circuit obfuscation framework for EVM using lattice assumptions, achieving estimated ${\sim}2^{98}$ post-quantum security (heuristic, see \S\ref{sec:lwe-params}) at ${\sim}8.5$M gas with $n{=}128$ and full-rank SEH. The three-layer design (topology + \lwe{} control function hiding + full-rank linear SEH) provides resistance against structural, statistical, semantic, and mix-and-match attacks.
...
Future work includes higher security parameters ($n \geq 256$), succinct verification via zkSNARK/STARK, and formal security analysis of batch SEH.
