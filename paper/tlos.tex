...
\tlos{} achieves 10.5M--38.1M gas (17--63\% of block, depending on circuit size) with $n{=}768$ LBLO (LWE-like) dimension and full-rank $64{\times}64$ wire binding. Security estimates ($2^{120\text{--}140}$ PQ) are based on heuristic modeling of our noiseless LBLO construction; see \S\ref{sec:lwe-params} for full parameterization. Storage is 11 bytes/gate via seed-derived $a$ vectors. Target applications include predicates with eventually-expiring secrets, particularly \emph{low-entropy secrets} (passwords, phrases, small ranges) where \tlos{} provides significant advantage over simple hash commitments by making each guess computationally expensive.
\end{abstract}
...
\item \textbf{LBLO (LWE-like, noiseless):} On-chain inner products hide control functions. \emph{Security: heuristic estimate $2^{120\text{--}140}$ PQ for our LBLO parameters; not standard LWE (see \S\ref{sec:lwe-params}).}
...
\begin{enumerate}
\item \textbf{\tlos{} Framework:} Three-layer obfuscation for EVM using lattice assumptions (10.5M--38.1M gas with $n{=}768$).
...
\midrule
Random 256-bit & $2^{256}$ hashes & $\min(2^{256}, 2^{120\text{--}140})$\textsuperscript{$\dagger$} \\
...
\bottomrule
\end{tabular}
\caption{Offline brute-force cost comparison. \textsuperscript{$\dagger$}For high-entropy secrets, \tlos{} security is bounded by the conjectured hardness of our LBLO construction (heuristic estimate $2^{120\text{--}140}$ with $n{=}768$; see \S\ref{sec:lwe-params}). Assumptions: keccak at $10^9$ hashes/s, \tlos{} at 100\,ms/guess.}
...
\item \textbf{Gas is cheap:} At 0.03 gwei, 19.3M gas (128 gates) $\approx$ \$1.80
\end{itemize}
...
\begin{tabular}{@{}lccc@{}}
\toprule
$n$ & Rate (ops/s) & Full $2^n$ time & Extrapolated $n{=}768$ \\
\midrule
16 & 1,500 & 44s & N/A \\
20 & 1,100 & 16min & N/A \\
24 & 820 & 5.7h & N/A \\
768 & N/A & N/A & $> 10^{200}$ years \\
\bottomrule
\end{tabular}
\end{center}
...
\noindent\textbf{Caveat:} Absence of known attacks $\neq$ proven security. The $2^{120\text{--}140}$ estimate is a \emph{heuristic} lower bound based on the best known attacks, not a formal security guarantee. Standard LWE reductions do not apply to our noiseless construction. We encourage independent cryptanalysis of the LBLO problem; our attack suite is available at \texttt{scripts/lblo\_attack.py}.
...
\midrule
CF hiding & LBLO (noiseless LWE-like, $n{=}768$) & $2^{120\text{--}140}$ PQ (heuristic) \\
...
We estimate $2^{120\text{--}140}$ post-quantum cost against the underlying LBLO instances by heuristically modeling them as LWE$(n{=}768, q{=}65521, m{=}2560)$ in standard lattice estimators. This should be interpreted as a rough complexity \emph{yardstick}, not a formal LWE-based security guarantee (see \S\ref{sec:lwe-params}). This security level is suitable for:
\begin{itemize}
...
For even higher security margins, $n \geq 1024$ could be considered (requires a high-throughput L2 or dedicated precompiles).
...
\textbf{Gas costs (measured, $n{=}768$):}
\begin{itemize}
\item \textbf{64 gates}: 10.5M gas (17\% of block)
\item \textbf{128 gates}: 19.3M gas (32\% of block)
\item \textbf{256 gates}: 38.1M gas (63\% of block)
\end{itemize}
...
\item $n{=}768$ LWE dimension for $2^{120\text{--}140}$ PQ security (heuristic)
\end{itemize}
...
\tlos{} is a practical circuit obfuscation framework for EVM using lattice-inspired assumptions, achieving estimated $2^{120\text{--}140}$ post-quantum security (heuristic, see \S\ref{sec:lwe-params}) at 10.5M--38.1M gas with $n{=}768$ and full-rank wire binding. The three-layer design (topology + LBLO (LWE-like) control function hiding + full-rank wire binding) provides resistance against structural, statistical, semantic, and mix-and-match attacks.
