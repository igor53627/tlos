...
Topology + \lwe{} + Wire Binding & 3/3 & 1/1 & 1/1 \\
\bottomrule
\end{tabular}
\end{center}

\textbf{Key findings:} (1)~Topology alone defeats structural attacks but not semantic (RainbowTable) attacks. (2)~\lwe{} is necessary to hide control functions from semantic analysis. (3)~Wire binding is necessary to prevent mix-and-match---without it, an attacker can evaluate gates with inconsistent wire values.

%------------------------------------------------------------------------------
\section{Conclusion}
%------------------------------------------------------------------------------

\tlos{} is a practical circuit obfuscation framework for EVM using lattice assumptions, achieving estimated ${\sim}2^{98}$ post-quantum security (heuristic, see \S\ref{sec:lwe-params}) at ${\sim}8.5$M gas with $n{=}128$ and full-rank wire binding. The three-layer design (topology + \lwe{} control function hiding + full-rank wire binding) provides resistance against structural, statistical, semantic, and mix-and-match attacks.
